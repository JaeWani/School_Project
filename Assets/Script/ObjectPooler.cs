...
public class ObjectPooler : Singleton<ObjectPooler>
{
	...
	public static GameObject SpawnFromPool(string tag, Vector3 position) => 
		I._SpawnFromPool(tag, position, Quaternion.identity);

	public static GameObject SpawnFromPool(string tag, Vector3 position, Quaternion rotation) => 
		I._SpawnFromPool(tag, position, rotation);

	public static T SpawnFromPool<T>(string tag, Vector3 position) where T : Component
	{
		GameObject obj = I._SpawnFromPool(tag, position, Quaternion.identity);
	...
	{
		GameObject obj = I._SpawnFromPool(tag, position, rotation);
		if (obj.TryGetComponent(out T component))
			return component;
		else
		{
	...
	{
		if (!I.poolDictionary.ContainsKey(tag))
			throw new Exception($"Pool with tag {tag} doesn't exist.");

		return I.spawnObjects.FindAll(x => x.name == tag);
	}
	...
	{
		if (!I.poolDictionary.ContainsKey(obj.name))
			throw new Exception($"Pool with tag {obj.name} doesn't exist.");

		I.poolDictionary[obj.name].Enqueue(obj);
	}
